...
async function selectActivity(slug) {
  ...
  const activity = await getJson(`/api/activities/${slug}`);
...
async function loadApp() {
  ...
  state.activities = await getJson("/api/activities");
  state.filteredActivities = state.activities.slice();
  renderStatus();
  filterActivities();

  const preferred = state.activities.find((activity) => activity.supported && !activity.simulation_disabled)
...
elements.simulationForm.addEventListener("submit", async (event) => {
  ...
  if (!response.ok) {
    elements.simulationResults.innerHTML = `<div class="results-empty">${result.error || "Simulation failed."}</div>`;
    return;
  }
